...
async fn get_following_index(
	...
	let total_items: i64 = sqlx::query("SELECT COUNT(1) FROM follows WHERE user_id = $1")
		.bind(user_id)
		.fetch_one(&state.db)
		.await?
		.get(0);
...
async fn get_following_first_page(
	...
	let rows = sqlx::query(
        "SELECT follows.following_since, users.this_instance, users.instance_url FROM follows, users WHERE follows.subject_user_id = $1 AND follows.subject_user_id = users.id ORDER BY follows.following_since DESC LIMIT 20",
    )
    .bind(user_id)
    .fetch_all(&state.db)
    .await?;
...
async fn get_following_max_timestamp(
	...
	let part_of_id_url = format!("{}/following", url::activitypub_actor(&username));
	...
	let rows = sqlx::query("SELECT follows.following_since, users.this_instance, users.instance_url FROM follows, users WHERE follows.subject_user_id = $1 AND follows.following_since < $2 AND follows.subject_user_id = users.id ORDER BY follows.following_since DESC LIMIT 20")
        .bind(user_id)
        .bind(max_timestamp)
        .fetch_all(&state.db)
        .await?;
...
async fn get_following_min_timestamp(
	...
	let rows = sqlx::query("SELECT * FROM (SELECT follows.following_since, users.this_instance, users.instance_url FROM follows, users WHERE follows.subject_user_id = $1 AND follows.following_since > $2 AND follows.subject_user_id = users.id ORDER BY follows.following_since LIMIT 20) AS tmp ORDER BY following_since DESC")
        .bind(user_id)
        .bind(min_timestamp)
        .fetch_all(&state.db)
        .await?;
